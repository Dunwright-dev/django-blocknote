...
from django_blocknote.models import UnusedImageURLS as img_model
...
def get_storage_class():
    ...
    if dj_bn_img_storage_setting:
        storage_class = dj_bn_img_storage_setting
...
def handle_uploaded_image(request):
    ...
    if getattr(settings, "DJ_BN_IMAGE_FORMATTER", None):
        convert_image = import_string(settings.DJ_BN_IMAGE_FORMATTER)
    else:
        convert_image = convert_image_to_webp
    if getattr(settings, "DJ_BN_IMAGE_URL_HANDLER", None):
        get_image_url_and_optionally_save = import_string(
            settings.DJ_BN_IMAGE_URL_HANDLER,
        )
    else:
        get_image_url_and_optionally_save = None

    if settings.DJ_BN_FORMAT_IMAGE and convert_image:
        file_name, image = convert_image(image)
    else:
        file_name = image.name

    # Set up to optionally save/return url to blocknote text editor.
    ...
    return image_url  # Return the saved image URL to the blocknote editor
